        ...
        >
            ...
            <div className="absolute bottom-0 flex gap-0 z-10 justify-center w-full px-5">
                {videos.map((_, idx) => (
                    <button
                        key={idx}
                        className={`${idx === 0 && "rounded-l-lg"} ${idx === videos.length - 1 && "rounded-r-lg"} z-100 w-[calc(100vw-16rem)] h-2 cursor-pointer ${idx === current ? "bg-[radial-gradient(circle_at_center,rgba(0,255,0,0.7),rgba(0,255,0,0.5),rgba(156,163,175,0.5))]" : "bg-gray-400/50"}`}
