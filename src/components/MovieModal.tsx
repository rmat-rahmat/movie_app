...
const MovieModal: React.FC<MovieModalProps> = ({ movie, onClose,showPlayback }) => {
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center">
      ...
      <div className="fixed inset-0 shadow-lg shadow-green-500/50 bg-black/70 rounded-lg my-auto md:h-[62%] md:mx-auto md:w-[80%] lg:w-[70%] xl:w-[60%] 2xl:w-[50%] overflow-y-auto z-61 flex flex-col items-center justify-center p-4 overflow-y-hidden">
        <div className="flex flex-col md:flex-row items-center mt-4">
          ...
          {!showPlayback &&  <>
            <img className="w-full rounded-lg md:hidden" src={movie.backdrop_path} alt={movie.original_title} />
            <img className="w-1/3 rounded-lg hidden md:block" src={movie.poster_path} alt={movie.original_title} /></>}

          <div className="ml-4">
            {showPlayback && <div className="flex items-center mt-2 mb-2">
            <iframe className="z-0 rounded-t-lg" width="100%" height="200" src={`https://www.youtube.com/embed/${movie.id}?controls=0&autoplay=1`} title={movie.original_title} allowFullScreen></iframe>
          </div> }

            <h2 className="text-2xl font-bold text-white">{movie.original_title}</h2>
            <p className="text-sm text-gray-400">{movie.release_date}</p>
            <br />
            <p className="text-sm text-white">
              {movie.overview.split(" ").slice(0, 100).join(" ") + (movie.overview.split(" ").length > 100 ? "..." : "")}
            </p>
              ...
              <div className="flex items-center mt-2">
                {[...Array(5)].map((_, index) => (
                  <svg
                    ...
                    className={`h-5 w-5 ${index < Math.ceil((movie.vote_average || 0) / 2) ? 'text-green-500' : 'text-gray-300'}`}
              ...
              <div className="grid grid-flow-col auto-cols-[15%] md:auto-cols-[17%] gap-4 mt-4">
                {movie.casts.slice(0, 5).map(cast => (
                  <div key={cast.id} className="flex flex-1 flex-col items-center">
                    <img className="w-full h-auto rounded-lg" src={cast.profile_path} alt={cast.name} loading="lazy" />
