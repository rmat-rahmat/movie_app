...
export default function LanguageSwitcher({ large }: { large?: boolean }) {
  ...
  } bg-gray-800 rounded-md shadow-lg z-50 border border-gray-700 max-h-60 overflow-y-auto`;

  return (
    <div ref={containerRef} className="relative inline-block text-left border-2 border-[#fbb033] rounded-full hover:bg-gray-700">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`${btnBase} ${btnSize} text-gray-300 cursor-pointer`}
      >
          ...
          <div className="py-1">
            {languageList.map((language) => (
              <button
                ...
                className={`flex items-center space-x-3 w-full px-4 py-2 text-left cursor-pointer hover:bg-gray-700 transition-colors ${
