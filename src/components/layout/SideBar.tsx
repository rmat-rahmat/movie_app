...
const SideBar = ({ show, hide }: { show: boolean, hide: () => void }) => {
  ...
  return (
    <div className={`pt-18 md:pt-25 bg-black/80 fixed lg:relative h-screen text-white py-6 z-49  ${show ? 'w-[70vw] lg:w-[20vw]' : 'w-0'} transition-width duration-300 ease-in-out`}>
      <div className={`fixed bg-black  px-4 h-full ${show ? 'w-[70vw] lg:w-[20vw]' : 'w-0 hidden'} transition-width duration-300 ease-in-out`}>
        ...
        <div className="absolute bg-black bottom-40 md:bottom-30 left-4 w-[90%]">
          {isAuthenticated ? (
